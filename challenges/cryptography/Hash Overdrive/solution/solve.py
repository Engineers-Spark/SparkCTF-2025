...
def extend_md(Data,Signature,Append,Key_length):
    try:
        new_message,to_hash = New(Data,Append,Key_length,64,8,"little")
        A,B,C,D = Lendian_STATE(Signature)
        new_hash = MD5(to_hash,(A,B,C,D))

        return (str(new_message)[2:-1],new_hash)
...
print(io.recv().decode())
messege = 'spark'
io.sendline(messege.encode())
Signature = io.recv().decode().split("\n")[0].split(" ")[-1]

new_messege , new_signature = extend_md(
    messege.encode(),
    Signature,
    b"admin",
    21,
...
io.sendline(b'2')
print(io.recv().decode())
io.sendline(new_messege.encode())
print(io.recv().decode())
